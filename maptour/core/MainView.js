...
//								app.data.getSourceLayer().queryFeatures(
									queryCount,
									function(nbFeature) {
...
//										FSManager.processFeature(nbFeature, app.data.isFSWithURLFields());
										
//										console.log(nbFeature);
										//console.log(tourPoints);
										features = nbFeature;
										//	https://mapping.chesterfield.mo.us/secure/rest/services/Plan/APTestCentroid/FeatureServer/0
										
	
//										tourPointLayerLoaded(nbFeature);
						...
						var featureLayer = new FeatureLayer("https://mapping.chesterfield.mo.us/secure/rest/services/Plan/APTestCentroid/FeatureServer/0",{
							//mode: FeatureLayer.MODE_ONDEMAND,
							//infoTemplate: infoTemplate,
							outFields: ["*"]
						  });	
////						FSManager.process(app.data.getSourceLayer(), app.data.isFSWithURLFields());
//						FSManager.process(featureLayer, app.data.isFSWithURLFields());
...
//						if ( app.data.getSourceLayer().objectIdField == "OBJECTID" && app.map.loaded ) {
...
//					app.data.setTourPoints(result.features.slice(0, APPCFG.MAX_ALLOWED_POINTS));
...
//							app.map.setZoom(app.map.getLevel() + 1).then(function(){
						...
						esriConfig.defaults.map.zoomDuration = 500;
						console.log("setting zoom level in else ");
					}
				}
//				app.map.setLevel(13);
				app.map.setLevel(1);
			...
			function selectedPointChange_after(forcedRecord)
			{
				...
				var more = (attributes.OBJECTID == 89)?'':' <a style = "float:right;" target="_blank" href="https://www.chesterfield.mo.us/test-active-projects.html?pid='+attributes.OBJECTID+'"> more info </a> ';
				app.desktopPicturePanel.updatePicture(
					attributes.getURL(),
					attributes.getName(),
					attributes.getDescription() + more,
